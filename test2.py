...
def move_notes():
    global combo, y_position, bottom_of_screen, player_hp, game_state  
...
def draw_win_screen():
    ...
    combo_text = combo_font.render(f"Max Combo: {combo}", True, black)
...
while running:
    ...
    if game_state == "playing":
                        ...
                        if y_position <= line_y and y_position + note_height >= line_y:
                            if lane == button_data.index(button):
                                ...
                                if combo % 10 == 0 and combo != 0:
                                    score += 5
                                note_data.remove(note)
                            elif y_position >= check_line:  # 노트가 check_line을 넘어간 경우 콤보 초기화
                                combo = 0
                                note_data.remove(note)
                            break
        ...
        combo_text = combo_font.render("COMB", True, black)   
        combo_rect = combo_text.get_rect(right=screen_width-140, top=450)
        pygame.draw.circle(screen, black, combo_rect.center, combo_rect.width-30, 8)
